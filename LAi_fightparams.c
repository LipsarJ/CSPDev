...
float LAi_CalcDamageForBlade(aref attack, aref enemy, string attackType, bool isBlocked, bool blockSave)
{
	...
	float fWeightCoeff;
	float fDecriment = frand(eSkill-aSkill);
	float bladeDmg = 0.4 *(max - decriment) * (aSkill + fDecriment) + 0.6  * (max - decriment) ;  
	
	if (aSkill < eSkill)
	{
		float reduceDmg = 1.0 - eSkill + aSkill;
		bladeDmg = bladeDmg * reduceDmg;
	}
	...
	case "fast":
		...
		else
		{
			...
			case "Fencing":
				kAttackDmg = 1.3;
				if (rand(100) < 5 && CheckAttribute(enemy, "cirassId"))
				{
					cirign = true;
					kAttackDmg += cirass_break(enemy, attack, fencing_type);
				}
				break;
			case "FencingHeavy":
				kAttackDmg = 0.9;
	...
	case "force":
		...
		else
		{
			...
			case "FencingLight":
				kAttackDmg = 1.2;
				if (rand(100) < 4 && CheckAttribute(enemy, "cirassId"))
				{
					cirign = true;
					kAttackDmg += cirass_break(enemy, attack, fencing_type);
				}
	...
	case "break":
		if (isBlocked)
		{
			...
			if (blockSave)
			{
				...
				case "FencingHeavy":
					kAttackDmg = 1;
					if (rand(100) < 30 && CheckAttribute(enemy, "cirassId"))
					{
						cirign = true;
						kAttackDmg += cirass_break(enemy, attack, fencing_type);
					}
			...
			else
			{
				...
				case "FencingHeavy":
					kAttackDmg = 1.8;
					if (rand(100) < 10 && CheckAttribute(enemy, "cirassId"))
					{
						cirign = true;
						kAttackDmg += cirass_break(enemy, attack, fencing_type);
					}
		...
		else
		{
			...
			case "FencingHeavy":
				kAttackDmg = 2.2;
				if (rand(100) < 15 && CheckAttribute(enemy, "cirassId"))
				{
					cirign = true;
					kAttackDmg += cirass_break(enemy, attack, fencing_type);
				}
	...
	if (kAttackDmg > 0) // оптимизация boal
	{
		if (attack.model.animation == "mushketer")
		{
			...
			if (at != "")
			{
				if (at == "break" && attackType == at)
					kAttackDmg += 1.5;
				if (at == "force" && attackType == at)
					kAttackDmg += 1.0;
			}
		}
...
if (isBlocked && blockSave)
	...
	float kDmg = 1.0;
	if(IsCharacterPerkOn(attack, "Rush")) kDmg = 1.5;		
   ...
   if (critchance > 0 && rand(100 - GetCharacterSPECIALSimple(attack, SPECIAL_L)) <= critchance) critical = 1.0;
	...
	if (dmg > 0 && !blockSave)
	{
		if(cirign || !CheckAttribute(enemy,"cirassId"))
		{
			if (HasSubStr(attack.equip.blade, "blade32")  && 12.5+valueB+ coeff + 2.5 > rand(99))//фламберж
			{
		...
		if (fencing_type == "FencingLight" && attackType == "force" && !HasSubStr(attack.equip.blade, "blade32"))//выпад
		{
				...
				if (valueT != 0)			 
				{
					if (rand(99)<valueT && !blockSave)
					{
						...
						if(sti(enemy.index) == GetMainCharacterIndex())
						{
							...
							PlaySound("interface\HrustBones_"+rand(2)+".wav");
							if (GetHealthName(enemy) != "ОТЛИЧНОЕ")
							{
	...
	{
		string cirassId = Items[sti(enemy.cirassId)].id;
		switch (cirassId)
		{
			case "cirass1":
				if (rand(9) < 4)
	...
	if(LAi_IsDead(enemy) && isSetBalde)
	{
		...
		if (stf(enemy.rank) < 4) experience = GetCharacterSPECIALSimple(attack, SPECIAL_I) + rand(GetCharacterSPECIALSimple(attack, SPECIAL_A));
...
void LAi_ApplyCharacterFireDamage(aref attack, aref enemy, float kDist)
{
	...
	if(LAi_IsDead(enemy) && isSetBalde)
	{
		...
		if (stf(enemy.rank) < 4) experience = GetCharacterSPECIALSimple(attack, SPECIAL_P) + rand(GetCharacterSPECIALSimple(attack, SPECIAL_I));
...
//функция пробития кирассы Lipsar
float cirass_break(aref enemy, aref attack, string fencing_type)
{
	if (IsCharacterPerkOn(attack, "Sliding") && Items[sti(enemy.cirassId)].id != "suit_1" && Items[sti(enemy.cirassId)].id != "suit_2" && Items[sti(enemy.cirassId)].id != "suit_3" && attack.chr_ai.group != enemy.chr_ai.group)
	{
		if (sti(attack.index) == GetMainCharacterIndex())
		{
			if (IsCharacterPerkOn(attack, "Sliding"))
			{
				...
				{
				case "FencingHeavy":
						Log_Info("Ваша атака пробила кирасу противника.");
						AddCharacterExpToSkill(attack, SKILL_FORTUNE, 20);
						PlaySound("interface\Breaking_" + rand(5) + ".wav");
						return 0.2;
					break;
				case "FencingLight":
					Log_Info("Вы попали в уязвимое место противника.");
					AddCharacterExpToSkill(attack, SKILL_FORTUNE, 20);
					PlaySound("interface\Breaking_" + rand(5) + ".wav");
					return 0.2;
					break;
				case "Fencing":
					Log_Info("Вы повредили кирасу противника.");
					AddCharacterExpToSkill(attack, SKILL_FORTUNE, 20);
					PlaySound("interface\Breaking_" + rand(5) + ".wav");
					return 0.2;
					break;
		...
		{
			if (enemy.Name == pchar.Name)
			{
				switch (fencing_type)
				{
				case "FencingHeavy":
						Log_Info("Атака по вам пробила кирасу.");
						AddCharacterExpToSkill(attack, SKILL_FORTUNE, 20);
						cirbreak += GetCharacterSkillSimple(enemy, "FencingHeavy") / 20;
						PlaySound("interface\Breaking_" + rand(5) + ".wav");
						return 0.2;
					break;
				case "FencingLight":
					Log_Info("Атака по вам попала в уязвимое место.");
					AddCharacterExpToSkill(attack, SKILL_FORTUNE, 20);
					cirbreak += GetCharacterSkillSimple(enemy, "Fencing") / 30;
					PlaySound("interface\Breaking_" + rand(5) + ".wav");
					return 0.2;
					break;
				case "Fencing":
					Log_Info("Атака по вам повредила кирасу.");
					AddCharacterExpToSkill(attack, SKILL_FORTUNE, 20);
					cirbreak += GetCharacterSkillSimple(enemy, "Fencing") / 20;
					PlaySound("interface\Breaking_" + rand(5) + ".wav");
					return 0.2;
					break;
				}
			...
			else
			{
				...
				{
				case "FencingHeavy":
						Log_Info("Атака по персонажу " + enemy.Name + " пробила кирасу.");
						AddCharacterExpToSkill(attack, SKILL_FORTUNE, 20);
						PlaySound("interface\Breaking_" + rand(5) + ".wav");
						return 0.2;
					break;
				case "FencingLight":
					Log_Info("Атака по персонажу " + enemy.Name + " попала в уязвимое место.");
					AddCharacterExpToSkill(attack, SKILL_FORTUNE, 20);
					PlaySound("interface\Breaking_" + rand(5) + ".wav");
					return 0.2;
					break;
				case "Fencing":
					Log_Info("Атака по персонажу " + enemy.Name + " повредила кирасу.");
					AddCharacterExpToSkill(attack, SKILL_FORTUNE, 20);
					PlaySound("interface\Breaking_" + rand(5) + ".wav");
					return 0.2;
					break;
